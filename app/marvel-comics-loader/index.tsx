import { Link, useLoaderData, useRouter } from 'expo-router';
...
export default function MarvelComicsIndex() {
  ...
  const [comicId, setComicId] = useState('');
    ...
    const goToComic = () => {
      ...
      if (!trimmed) return;
      router.push(`/marvel-comics-loader/${trimmed}`);
    };
  ...
  return (
    <>
      ...
      {data.items.map((comic: Comic) => (
        <View key={comic.id} style={styles.comic}>
          <Link href={`/marvel-comics-loader/${comic.id}`}>
            <Link.Trigger>
