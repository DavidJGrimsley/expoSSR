...
export async function loader(request: Request | undefined, params: { id: string }) {
  ...
  if (origin) {
    const response = await fetch(`${origin}/api/blog/${encodeURIComponent(params.id)}`);
    ...
    if (!result.success || result.kind !== 'detail') {
      throw new Error(result.error || `Post with ID "${params.id}" not found`);
    }
  ...
  return (
    <ScrollView contentContainerStyle={styles.errorContainer}>
      ...
      <View style={styles.errorNote}>
        ...
        <Text style={styles.errorNoteText} style={{ marginTop: 8 }}>
